...
describe('For the route for creating a todo POST:(/todo)', () => {
    ...
    it('it should return {success: true, data: (new todo object)} and has a status code of 200 when called using POST', async () => {
        ...
        //another happy path but different way of sending data
    it('it should return {success: true, data: (new todo object)} and has a status code of 200 when called using POST even if we dont provide the done property. Default of done should still be false.', async () => {
        const response = await app.inject({
            method: 'POST',
            url: '/todo',
            payload: {
